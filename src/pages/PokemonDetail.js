...
import { DetailPills, ModalLoading } from "../components"
import { useDispatch } from "react-redux"
import { ADD_POKEMON, EDIT_POKEMON } from "../store/action"
...
export default function PokemonDetail() {
  ...
  const [ableToChangeName, setAbleToChangeName] = useState(false)
  ...
  async function addFavorite () {
    ...
    if(random === 1) {
      let merge = {...pokemon, ...pokemonDetail}
      await dispatch(ADD_POKEMON(merge))
      setAbleToChangeName(true)
      setTimeout(function(){ setLoadingCatch(true) }, 3000)
      setTimeout(function(){ setCatched(true) }, 3000)
      setTimeout(function (){
        history.push(`/my-pokemon-list/${inputName}`, merge)
      }, 6000)
    }
    else {
      setAbleToChangeName(false)
      setTimeout(function(){ setLoadingCatch(true) }, 3000)
      setTimeout(function(){ setCatched(false) }, 3000)
    }
  }
  ...
  async function handleSubmit (e) {
    ...
    await dispatch(EDIT_POKEMON(name, inputName))
    setShowForm(false)
  }

  return (
    <div data-testid="pokemon-detail-page" className="mx-auto space-y-16 sm:grid sm:grid-cols-3 sm:gap-16 sm:space-y-0 lg:max-w-5xl">
      ...
      <ModalLoading loading={loadingCatch} catched={catched} setLoading={setLoadingCatch} setCatched={setCatched}/>
    </div>
  )
}